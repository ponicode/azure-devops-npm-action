...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--azure_url", dest="azure_url",required=False, help="Your TFS instance")
    return parser.parse_args()

def generate_url(args: argparse.Namespace):
    devopsUrl = "pkgs.dev.azure.com"
    if args.azure_url is not None:
        devopsUrl = args.azure_url

    if args.project is not None:
        return f"{devopsUrl}/{args.organisation}/{args.project}/_packaging/{args.registry}/npm"

    return f"{devopsUrl}/{args.organisation}/_packaging/{args.registry}/npm"
